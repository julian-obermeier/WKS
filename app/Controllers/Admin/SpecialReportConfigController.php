...
final class SpecialReportConfigController
{
    ...
    public function saveType(Request $request): Response
    {
        ...
        $allowedForce=['action_type','started_at','ended_at','staff_ids','justification','result_text'];
        $forceRequirements=[];foreach((array)$request->post('force_required',[]) as $field)if(in_array((string)$field,$allowedForce,true))$forceRequirements[(string)$field]=true;
        $data=['name'=>$name,'code'=>preg_replace('/[^a-z0-9_]/','_',strtolower($code)),'sort_order'=>(int)$request->post('sort_order',0),'active'=>$request->post('active')?1:0,'force_section_enabled'=>$request->post('force_section_enabled')?1:0,'force_requirements_json'=>json_encode($forceRequirements,JSON_THROW_ON_ERROR|JSON_UNESCAPED_UNICODE)];
